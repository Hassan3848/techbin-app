...
function disposalRouteLabel(disposedSide: unknown, expectedSide: unknown) {
  const disposed = sideLabel(disposedSide);
  const expected = expectedSide ? sideLabel(expectedSide) : "-";
  return `${disposed} / expected ${expected}`;
}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-100">
          <h2 className="text-lg text-gray-900">Recent Disposal Events</h2>
          <p className="text-sm text-gray-600 mt-1">Placement-unconfirmed events are shown here even when overview counters look unchanged.</p>
        </div>
              ...
              {loading ? (
                ...
                return (
                  <tr key={event.id || index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-700">{disposalRouteLabel(event.disposedSide, event.expectedSide)}</td>
